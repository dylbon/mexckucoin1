...
def fetch_kraken_tickers():
    ...
    try:
        ...
        print(f"Kraken: {len(prices)} assets")
...
def check_arbitrage():
    ...
    for base_raw in all_bases:
        ...
        k_eur = k['bid_eur'] if k and 'bid_eur' in k else None
        ...
        for o in opps:
            ...
            msg = (
                ...
                f"→ **{o['profit']:.2f}%** profit (raw spread {o['raw']:.2f}%)"
            )
            print(msg + "\n")
            send_telegram(msg)

    print(f"Found {found} opportunities" if found else f"No ≥{THRESHOLD_PERCENT}% ops")
...
if __name__ == "__main__":
    ...
    print("Running — min raw spread 4%, max 200%, EUR basis\n")
    while True:
        try:
            check_arbitrage()
